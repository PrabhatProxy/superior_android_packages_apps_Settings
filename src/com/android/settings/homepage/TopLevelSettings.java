...
public class TopLevelSettings extends DashboardFragment implements SplitLayoutListener,
    ...
    private static final String KEY_SUPERIORLAB = "top_level_superior_lab";
    ...
    private void updateLabSummary() {
        Preference superiorlab = findPreference(KEY_SUPERIORLAB);
        if (superiorlab != null) {
            String[] summaries = getContext().getResources().getStringArray(
                    R.array.superiorlab_summaries);
            Random rnd = new Random();
            int summNO = rnd.nextInt(summaries.length);
            superiorlab.setSummary(summaries[summNO]);
        }
    }
